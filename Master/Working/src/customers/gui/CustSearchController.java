...
public class CustSearchController implements Initializable {
    ...
    @FXML
    public void showBusiness(ActionEvent event) throws SQLException
    {
        Cust.getItems().clear();
        Context ct=Context.getInstance();
        String name = ct.getSearch();
        ...
        try {
            if(ct.getCriteria().equals("Name"))
            {
                a=(new DataFunc()).getSearchCritBusiness(name);
            }
            else
            {
                a=(new DataFunc()).getSearchRegBusiness(name);
            }
        } catch (SQLException ex) {
            Logger.getLogger(CustSearchController.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    @FXML
    public void showIndiv(ActionEvent event) throws SQLException
    {
        Cust.getItems().clear();
        Context ct=Context.getInstance();
        String name = ct.getSearch();
        ...
        try {
            if(ct.getCriteria().equals("Name"))
            {
                a=(new DataFunc()).getSearchCritIndividual(name);
            }
            else
            {
                a=(new DataFunc()).getSearchRegIndividual(name);
            }
        } catch (SQLException ex) {
            Logger.getLogger(CustSearchController.class.getName()).log(Level.SEVERE, null, ex);
        }
